...
/* TOPBAR */
.topbar{background:var(--brown);padding:13px 18px;display:flex;align-items:center;
  justify-content:space-between;border-bottom:2px solid var(--rust);flex-shrink:0;z-index:10}
...
.user-pill{font-family:var(--fm);font-size:9px;letter-spacing:1px;text-transform:uppercase;
  color:rgba(255,255,255,0.4);padding:5px 10px;border:1px solid rgba(255,255,255,0.15);border-radius:20px}
.btn{font-family:var(--fm);font-size:10px;letter-spacing:1px;padding:7px 14px;border-radius:6px;
  cursor:pointer;border:none;text-transform:uppercase;transition:all 0.2s}
...
.login-card{background:var(--card);border:1.5px solid var(--border);border-radius:16px;
...
.login-sub{font-family:var(--fm);font-size:10px;letter-spacing:2px;text-transform:uppercase;
  color:var(--muted);margin-bottom:28px}
.input{width:100%;padding:11px 14px;border:1.5px solid var(--border);border-radius:8px;
  font-family:var(--fb);font-size:14px;background:var(--warm);color:var(--text);
...
.section-label{font-family:var(--fm);font-size:10px;letter-spacing:3px;text-transform:uppercase;
...
.stat-card{background:var(--card);border:1.5px solid var(--border);border-radius:14px;
  padding:16px 14px 13px;position:relative;overflow:hidden;transition:transform 0.2s}
...
.badge{font-family:var(--fm);font-size:8px;letter-spacing:1px;padding:1px 7px;
  border-radius:20px;border:1px solid currentColor;color:var(--mustard)}

/* EVENTS */
.ev-list{display:flex;flex-direction:column}
.ev-item{display:grid;grid-template-columns:4px 1fr auto;align-items:center;
  gap:11px;padding:11px 0;border-bottom:1px solid var(--border)}
...
/* TASKS */
.task-group-lbl{font-family:var(--fm);font-size:9px;letter-spacing:2px;text-transform:uppercase;
  color:var(--muted);margin:16px 0 8px}
.task-list{display:flex;flex-direction:column;gap:7px}
.task-item{display:flex;align-items:center;gap:10px;padding:10px 13px;border-radius:8px;
  background:var(--warm);border:1px solid transparent;cursor:pointer;transition:all 0.2s}
.task-item:hover{border-color:var(--border)}
.task-item.done{opacity:0.45}
.task-check{width:18px;height:18px;border-radius:50%;border:2px solid var(--border);
  display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:10px;color:white;transition:all 0.2s}
...
.task-tag{font-family:var(--fm);font-size:9px;letter-spacing:1px;padding:2px 8px;
...
/* MUSIC PAGE */
.ep-banner{background:linear-gradient(135deg,var(--brown) 55%,var(--rust));border-radius:14px;
  padding:22px 18px;margin-bottom:18px;color:white;position:relative;overflow:hidden}
.ep-banner::after{content:'EP';position:absolute;right:-8px;top:-12px;font-family:var(--fd);
  font-size:80px;font-weight:900;color:rgba(255,255,255,0.05);line-height:1}
...
.track-item{display:grid;grid-template-columns:36px 1fr auto;align-items:center;gap:11px;
  padding:11px 13px;background:var(--warm);border-radius:10px;border:1px solid transparent;
...
.cal-day-name{font-family:var(--fm);font-size:9px;letter-spacing:1px;text-transform:uppercase;
  color:var(--muted);text-align:center;padding:3px 0}
.cal-day{aspect-ratio:1;display:flex;align-items:center;justify-content:center;border-radius:7px;
  font-size:12px;font-weight:600;cursor:pointer;transition:background 0.15s;color:var(--text);position:relative}
...
.upcoming-item{display:flex;gap:12px;align-items:flex-start;padding:11px 13px;
  background:var(--warm);border-radius:10px;margin-bottom:8px}
...
.msg.ai{align-self:flex-start;background:var(--card);border:1.5px solid var(--border);
  color:var(--text);border-bottom-left-radius:4px}
.msg.ai .msg-label{font-family:var(--fm);font-size:9px;letter-spacing:1px;text-transform:uppercase;
  color:var(--mustard);margin-bottom:5px}
.msg.loading{opacity:0.6}
.chat-input-wrap{padding:12px 16px;border-top:1.5px solid var(--border);background:var(--card);
  display:flex;gap:8px;align-items:flex-end}
.chat-input{flex:1;padding:10px 13px;border:1.5px solid var(--border);border-radius:10px;
  font-family:var(--fb);font-size:13px;background:var(--warm);color:var(--text);
  resize:none;outline:none;transition:border-color 0.2s;max-height:100px;min-height:40px}
.chat-input:focus{border-color:var(--rust)}
.chat-send{width:38px;height:38px;border-radius:9px;background:var(--rust);border:none;
...
.ai-chip{font-family:var(--fm);font-size:10px;letter-spacing:0.5px;padding:6px 12px;
  border:1.5px solid var(--border);border-radius:20px;cursor:pointer;transition:all 0.2s;
...
.modal{background:var(--card);border-radius:20px 20px 0 0;padding:24px 20px 36px;
  width:100%;max-width:500px;border-top:2px solid var(--rust)}
.modal-title{font-family:var(--fd);font-size:18px;font-weight:700;color:var(--brown);margin-bottom:18px}
.form-label{font-family:var(--fm);font-size:9px;letter-spacing:1px;text-transform:uppercase;
  color:var(--muted);margin-bottom:5px;display:block}
.form-group{margin-bottom:14px}
.select{width:100%;padding:10px 13px;border:1.5px solid var(--border);border-radius:8px;
  font-family:var(--fb);font-size:14px;background:var(--warm);color:var(--text);outline:none}
.modal-actions{display:flex;gap:8px;margin-top:20px}

/* BOTTOM NAV */
.bottom-nav{background:var(--brown);border-top:2px solid var(--rust);display:flex;
  align-items:center;justify-content:space-around;
  padding:7px 0 max(7px,env(safe-area-inset-bottom));flex-shrink:0;z-index:10}
.nav-item{display:flex;flex-direction:column;align-items:center;gap:2px;cursor:pointer;
  padding:5px 12px;border-radius:10px;transition:background 0.2s;flex:1}
...
.loading-screen{display:flex;align-items:center;justify-content:center;height:100%;
  background:var(--cream);flex-direction:column;gap:12px}
...
@media(min-width:640px){
  ...
  .content{padding:26px 26px 48px}
}
`
...
export default function App() {
  ...
  const [modal, setModal] = useState(null) // 'evento' | 'tarea' | null
  ...
  useEffect(() => { goTo(page, false) }, [])

  const onTouchStart = (e) => {
    startX.current = e.touches[0].clientX
    startY.current = e.touches[0].clientY
    dragging.current = true; delta.current = 0
  }
  const onTouchMove = (e) => {
  ...
  if (loading) return (
    <>
      ...
      <div className="loading-screen">
        <div className="loading-logo">ADA</div>
  ...
  if (!session) return (
    <>
      <style>{css}</style>
      <div className="login-wrap"><LoginCard /></div>
    </>
  )
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {/* PAGES */}
        <div className="pages-viewport" ref={vpRef}
          ...
          <div className="pages-track" ref={trackRef}>
              ...
              <div className="content">
                ...
                <div className="card">
                  <div className="card-header">
                    <span className="card-title">Hitos planificados</span>
                    <span className="card-yr">2026</span>
                  </div>
                  <div className="card-sub">Calendario de lanzamientos y actividades</div>
                  <div className="ev-list">
                    {eventos.slice(0, 5).map(ev => (
                      <div key={ev.id} className="ev-item">
                        <div className={`ev-bar eb-${ev.color}`} />
                        <div>
                          <div className="ev-name">{ev.nombre}</div>
                          {ev.detalle && <div className="ev-detail">{ev.detalle}</div>}
                        </div>
            ...
            <div className="page">
              <div className="content">
                ...
                <div className="section-label" style={{ marginTop: 18 }}>plataformas</div>
                <div className="card" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                  <div style={{ textAlign: 'center', padding: '12px 0' }}>
                    <div style={{ fontFamily: 'var(--fd)', fontSize: 26, fontWeight: 900, color: 'var(--rust)' }}>3M+</div>
                    <div style={{ fontFamily: 'var(--fm)', fontSize: 9, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--muted)', marginTop: 4 }}>Oyentes / mes</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: '12px 0', borderLeft: '1px solid var(--border)' }}>
                    <div style={{ fontFamily: 'var(--fd)', fontSize: 26, fontWeight: 900, color: 'var(--mustard)' }}>Récord</div>
                    <div style={{ fontFamily: 'var(--fm)', fontSize: 9, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--muted)', marginTop: 4 }}>Histórico mar 2026</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="content">
                ...
                {eventos.map(ev => (
                  <div key={ev.id} className="upcoming-item">
                    ...
                    <div>
                      <div className="upcoming-name">{ev.nombre}</div>
                      {ev.detalle && <div className="upcoming-detail">{ev.detalle}</div>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* PAGE 4: IA */}
            <div className="page">
              <AIPage eventos={eventos} tareas={tareas} metricas={metricas} />
            </div>
...
function CalendarGrid({ eventos }) {
  ...
  const today = 25 // mar 25, 2026 — abril starts Wed=3rd col
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="form-group">
          ...
          <input className="input" value={nombre} onChange={e => setNombre(e.target.value)} placeholder='Ej: Lanzamiento "Canción"' />
        </div>
        <div className="form-group">
          <label className="form-label">Detalle (opcional)</label>
          <input className="input" value={detalle} onChange={e => setDetalle(e.target.value)} placeholder="Artistas, venue, etc." />
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          ...
          <select className="select" value={color} onChange={e => setColor(e.target.value)}>
            ...
            <option value="rust">Óxido</option>
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="form-group">
          ...
          <input className="input" value={texto} onChange={e => setTexto(e.target.value)} placeholder="Describí la tarea..." onKeyDown={e => e.key === 'Enter' && save()} />
        </div>
        <div className="form-group">
...
function AIPage({ eventos, tareas, metricas }) {
  ...
  const context = `
Sos el asistente de ADA, una artista musical. Tenés acceso a su dashboard en tiempo real.

MÉTRICAS ACTUALES:
${metricas.map(m => `- ${m.titulo}: ${m.valor} (${m.subtitulo || ''})`).join('\n')}

PRÓXIMOS EVENTOS:
${eventos.map(e => `- ${e.nombre}${e.detalle ? ` (${e.detalle})` : ''} — ${e.fecha}`).join('\n')}

TAREAS:
${tareas.map(t => `- [${t.completada ? 'HECHO' : 'PENDIENTE'}] ${t.texto} (${t.prioridad})`).join('\n')}

Respondé siempre en español. Sé conciso, útil y con energía. Podés dar recomendaciones estratégicas sobre la carrera de ADA basándote en los datos que tenés.
  `.trim()
  ...
  const send = async (text) => {
    ...
    try {
      ...
      setMessages([...newMsgs, { role: 'assistant', content: 'Error de conexión. Intentá de nuevo.' }])
    }
    setLoading(false)
  }

  const chips = [
    '¿Cómo va la carrera de ADA?',
    '¿Qué tareas son más urgentes?',
    'Estrategia para el lanzamiento del EP',
    '¿Qué evento es el más importante?',
  ]
    ...
    <div className="ai-wrap">
      <div className="chat-messages">
        {messages.length === 0 && (
          <div className="ai-intro">
            ...
            <div className="ai-intro-sub">
              Tengo acceso a todas las métricas, eventos y tareas en tiempo real. Preguntame lo que quieras sobre la carrera.
            </div>
        ...
        {loading && (
          <div className="msg ai loading">
            <div className="msg-label">✦ ADA AI</div>
            Pensando...
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div className="chat-input-wrap">
        <textarea
          ...
          placeholder="Preguntale algo a ADA AI..."
          value={input}
